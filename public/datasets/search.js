  var options = {
    ...
    queries: { 
      ldcatalogResultset: 'PREFIX : <http://voc.zazuko.com/zack#> PREFIX  rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX  schema: <http://schema.org/> PREFIX  rdf:  <http://www.w3.org/1999/02/22-rdf-syntax-ns#> PREFIX  skos: <http://www.w3.org/2004/02/skos/core#> PREFIX  xsd: <http://www.w3.org/2001/XMLSchema#> CONSTRUCT {:query :result ?sub . ?sub a :resultItem . ?sub ?pred ?obj . ?obj schema:name ?oname.} WHERE {{ SELECT ?sub WHERE {{ SELECT ?sub WHERE { ?sub schema:name|schema:description ?name. ${textmatch} ${filters} } ORDER BY ?resultType ?name } }  OFFSET ${offset} LIMIT ${limit} } { ?sub ?pred ?obj . OPTIONAL {?obj schema:name ?oname.} } }',
      ldcatalogMeta: 'PREFIX : <http://voc.zazuko.com/zack#> PREFIX  rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX  schema: <http://schema.org/> PREFIX  rdf:  <http://www.w3.org/1999/02/22-rdf-syntax-ns#> PREFIX  skos: <http://www.w3.org/2004/02/skos/core#> PREFIX  xsd: <http://www.w3.org/2001/XMLSchema#> CONSTRUCT { _:b0 :numberOfResults ?count.  _:b0 :queryStart ?querystart.  _:b0 :queryEnd ?queryend.  } WHERE { { SELECT (COUNT(?sub) as ?count) { ?sub schema:name|schema:decription ?name. ${textmatch} ${filters} } } }',
